...
function BasketSummary() {
	...
	const totalWithoutDiscount =
		basket?.basketItems.reduce(
			(value, basketItem) => value + basketItem.quantity * basketItem.product?.price!,
			0
		) ?? 0;
	const discount =
		basket?.basketItems.reduce(
			(value, basketItem) =>
				value + (basketItem.quantity * basketItem.product?.price! * basketItem.product?.discount!) / 100,
			0
		) ?? 0;
		...
		<Box component={Paper} p={2} elevation={10}>
			...
			<List sx={{ width: "100%", maxWidth: "min(100%, 100vw)" }}>
				<ListItem sx={{ display: "flex" }}>
					...
					<ListItemText sx={{ flex: 2 }}>Rs. {(totalWithoutDiscount / 100).toFixed(2)}</ListItemText>
				</ListItem>
				<ListItem sx={{ display: "flex" }}>
					<ListItemText sx={{ flex: 1 }}>Discount</ListItemText>
					<ListItemText sx={{ flex: 2 }}>
						<Typography>Rs. {(discount / 100).toFixed(2)}</Typography>
					</ListItemText>
				</ListItem>
				<ListItem sx={{ display: "flex" }}>
					<ListItemText sx={{ flex: 1 }}>Delivery</ListItemText>
					<ListItemText sx={{ flex: 2 }}>Rs. {(delivery / 100).toFixed(2)}</ListItemText>
				</ListItem>
				<ListItem
					sx={{
						display: "flex",
					...
					<ListItemText sx={{ flex: 1 }}>
						<Typography fontSize="large" fontWeight="bold" color="primary.main">
							Total
						</Typography>
					</ListItemText>
					<ListItemText sx={{ flex: 2 }}>
						<Typography fontSize="medium">Rs. {((total + delivery) / 100).toFixed(2)}</Typography>
					</ListItemText>
				</ListItem>
			</List>
